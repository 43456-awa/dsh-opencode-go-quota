...
// ============== DOT-PATH RESOLVER ==============
/**
 * 按点路径取值，支持键名本身含点号（如 "glm-5.2"）：
 * 逐段解析，某段在对象中找不到时贪心拼接后续段再试。
...
function resolvePath(obj, path) {
  ...
  for (let i = 0; i < parts.length; i++) {
    let key = parts[i];
    let j = i;
    while (!(o && typeof o === "object" && key in o) && j + 1 < parts.length) {
      j++;
      key = key + "." + parts[j];
    }
  ...
  try {
    ...
    for (const w of (plan.windows || [])) {
      if (w.type === "balance") {
        ...
        const remaining = resolvePath(payload, w.balancePath);
        if (typeof remaining !== "number") { ok = false; break; }
        const total = w.totalPath ? resolvePath(payload, w.totalPath) : undefined;
        const used = w.usedPath ? resolvePath(payload, w.usedPath) : undefined;
        windows[w.name] = {
          kind: "balance",
          remaining,
