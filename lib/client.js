	...
	factory: (require) => {
		...
		function QuotaDot(props) {
			...
			return h("div", { "data-plugin": "dsh-opencode-go-quota", style: { position: "relative", display: "inline-flex", alignItems: "center", flex: "none" } },
				h("button", {
					...
					onClick: () => setView(view === "quota" ? "plans" : "quota"),
		...
		function PlansView(props) {
			...
			return h("div", { style: { ...panelStyle, overflowY: "auto" } },
				...
				plans && plans.length > 0 ? plans.map(p => h("div", {
					...
					h("button", { type: "button", title: "编辑", style: { ...btn, fontSize: 10 }, onClick: (e) => { e.stopPropagation(); onEdit(p); } }, "✎"),
		...
		function PlanEditor(props) {
			...
			return h("div", { style: { ...panelStyle, overflowY: "auto" } },
				...
				testResult ? h("div", { style: { fontSize: 10, padding: "4px 6px", borderRadius: 5, background: "var(--dsw-alias-interactive-bg-hover)", wordBreak: "break-all", maxHeight: 120, overflowY: "auto" } },
					h("div", { style: { color: testResult.success ? "var(--dsw-alias-state-success-primary, #10b981)" : "var(--dsw-alias-state-danger-primary, #ef4444)" } }, testResult.success ? "✓ 连接成功" : `✗ ${testResult.data?.status || "失败"}`),
					testResult.data?.raw ? h("pre", { style: { fontSize: 9, margin: "2px 0", whiteSpace: "pre-wrap", color: "var(--dsw-alias-label-secondary)" } }, testResult.data.raw.slice(0, 500)) : null) : null);
		}
