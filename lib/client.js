...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function QuotaDot(props) {
			const [view, setView] = useState("closed"); // closed | quota | plans | editor（默认收起，点圆圈展开）
			...
			// 圆圈颜色：percent 窗口按剩余比例，balance 窗口按 remaining/total（无 total 时中性色）
			let minRate = null;
			let tip = "加载中…";
			if (quotaData && quotaData.status === "active" && quotaData.windows) {
				const rates = [];
				for (const w of Object.values(quotaData.windows)) {
					if (!w) continue;
					if (w.kind === "balance") {
						if (typeof w.total === "number" && w.total > 0) rates.push(Math.max(0, Math.min(1, w.remaining / w.total)));
					} else if (typeof w.percentRemaining === "number") {
						rates.push(w.percentRemaining / 100);
					}
				}
			...
			return h("div", { "data-plugin": "dsh-opencode-go-quota", style: { position: "relative", display: "inline-flex", alignItems: "center", flex: "none" } },
				h("button", {
					type: "button", title: tip, "aria-label": "套餐额度",
					onClick: () => setView((v) => (v === "closed" ? "quota" : "closed")),
				...
				view !== "closed" && (view === "quota" || view === "plans") ? h(QuotaView, { quotaData, loading, error, lastRefresh, plans, activePlan, onRefresh: () => fetchQuota(activePlanRef.current, true), onSelectPlan: selectPlan, onAdd: () => { setEditPlan(null); setView("editor"); }, onEdit: (p) => { setEditPlan(p); setView("editor"); }, showPlans: view === "plans", onTogglePlans: () => setView(view === "plans" ? "quota" : "plans"), onBack: () => setView("quota") }) : null,
				view !== "closed" && view === "editor" ? h(PlanEditor, { plan: editPlan, onSaved: () => { loadPlans(); setView("quota"); }, onCancel: () => setView("quota") }) : null
			);
		}

		// ---- 额度视图（含可折叠套餐列表）-------------------------------
		function QuotaView(props) {
			const { quotaData, loading, error, lastRefresh, plans, activePlan, onRefresh, onSelectPlan, onAdd, onEdit, showPlans, onTogglePlans, onBack } = props;
			...
			const Bar = (label, win) => {
				// balance 型：通用余量（cc-switch 风格）
				if (win && win.kind === "balance") {
					const hasTotal = typeof win.total === "number" && win.total > 0;
					const ratio = hasTotal ? Math.max(0, Math.min(1, win.remaining / win.total)) : null;
					const color = stateColor(ratio);
					const unit = win.unit || "";
					const remainingText = fmtNum(win.remaining) + unit;
					const totalText = hasTotal ? fmtNum(win.total) + unit : null;
					return h("div", { style: { marginBottom: 10 } },
						h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 } },
							...
							h("span", { style: { fontSize: 11, fontWeight: 600, color } }, `剩余 ${remainingText}`)),
						hasTotal ? h("div", { style: { width: "100%", height: 6, background: "var(--dsw-alias-interactive-bg-hover)", borderRadius: 3 } },
							h("div", { style: { height: 6, borderRadius: 3, background: color, width: `${ratio * 100}%`, transition: "width 0.5s ease" } })) : null,
						h("div", { style: { display: "flex", justifyContent: "space-between", marginTop: 2 } },
							h("span", { style: { fontSize: 9, color: "var(--dsw-alias-label-tertiary)" } }, totalText ? `总额 ${totalText}` : (typeof win.used === "number" ? `已用 ${fmtNum(win.used)}${unit}` : "")),
							h("span", { style: { fontSize: 9, color: "var(--dsw-alias-label-tertiary)" } }, "")));
				}
				// percent 型
				const pct = win ? win.percentRemaining : 0;
				const color = stateColor(pct / 100);
				let rt = "--"; try { const d = new Date(win && win.resetsAt ? win.resetsAt : ""); if (!Number.isNaN(d.getTime())) rt = fmtShort(d); } catch {}
				return h("div", { style: { marginBottom: 10 } },
					h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 } },
						h("span", { style: { fontSize: 11, color: "var(--dsw-alias-label-secondary)" } }, label),
			...
			return h("div", { style: panelStyle },
				// Header
				h("div", { style: { display: "flex", alignItems: "center", gap: 6, marginBottom: 8, paddingBottom: 6, borderBottom: "1px solid var(--dsw-alias-border-l1)", flex: "none" } },
					...
					h("button", { type: "button", title: "管理套餐", style: { ...btn, fontSize: 11 }, onClick: onTogglePlans }, showPlans ? "▲" : "▼"),
				...
				quotaData?.status === "active" && quotaData.windows ? h("div", null, Object.entries(quotaData.windows).map(([k, w]) => Bar(k, w))) :
				...
				// 可折叠套餐列表
				showPlans ? h("div", { style: { borderTop: "1px solid var(--dsw-alias-border-l1)", paddingTop: 6, marginTop: 6 } },
					h("div", { style: { fontSize: 10, color: "var(--dsw-alias-label-secondary)", marginBottom: 4 } }, "套餐列表"),
					plans && plans.length > 0 ? plans.map(p => {
						var delPlan = function(id) {
							if (!confirm("确定删除此套餐？")) return;
							fetch(API_PATH + "/plans/" + id, { method: "DELETE" }).then(function() { onTogglePlans(); }).catch(function() {});
						};
						return h("div", { key: p.id, style: { display: "flex", alignItems: "center", gap: 6, padding: "5px 6px", borderRadius: 6, cursor: "pointer", marginBottom: 2, background: p.id === activePlan ? "var(--dsw-alias-interactive-bg-hover)" : "transparent" }, onClick: function(e) { if (e.target.closest("button")) return; onSelectPlan(p.id); onBack(); } },
							...
							h("div", { style: { flex: 1, minWidth: 0, fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, p.name),
							p.id === activePlan ? h("span", { style: { fontSize: 9, color: "var(--dsw-alias-label-tertiary)" } }, "✓") : null,
							h("button", { type: "button", title: "编辑", style: { ...btn, fontSize: 10 }, onClick: function(e) { e.stopPropagation(); onEdit(p); } }, "✎"),
							h("button", { type: "button", title: "删除", style: { ...btn, fontSize: 10, color: "var(--dsw-alias-state-danger-primary, #ef4444)" }, onClick: function(e) { e.stopPropagation(); delPlan(p.id); } }, "✕"));
					}) : h("div", { style: { padding: "6px 0", textAlign: "center", color: "var(--dsw-alias-label-secondary)", fontSize: 10 } }, "暂无套餐"),
					h("button", { type: "button", style: { width: "100%", padding: "4px", marginTop: 4, border: "1px dashed var(--dsw-alias-border-l1)", borderRadius: 5, background: "transparent", color: "var(--dsw-alias-label-primary)", cursor: "pointer", fontSize: 10 }, onClick: onAdd }, "+ 添加套餐"),
					h("div", { style: { fontSize: 9, color: "var(--dsw-alias-label-tertiary)", marginTop: 3, textAlign: "center" } }, "绿灯=已配token · 灰灯=未配置")
				) : null,
				// Footer
				h("div", { style: { display: "flex", justifyContent: "space-between", paddingTop: 6, borderTop: "1px solid var(--dsw-alias-border-l1)", color: "var(--dsw-alias-label-tertiary)", fontSize: 9, marginTop: 6 } },
		...
		function PlanEditor(props) {
			...
			const addWin = () => setWindows([...windows, { name: "", type: "percent", percentPath: "", resetsAtPath: "", invertPercent: false }]);
			...
			const save = async () => {
				...
				setSaving(true);
				try {
					const body = { name, apiUrl, token, authType, windows: windows.filter(w => w.name && (w.type === "balance" ? w.balancePath : w.percentPath)) };
			...
			return h("div", { style: { ...panelStyle, overflowY: "auto" } },
				...
				windows.map((w, i) => h("div", { key: i, style: { marginBottom: 6, padding: "4px 6px", border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 5 } },
					h("div", { style: { display: "flex", gap: 4, marginBottom: 3 } },
						h("input", { style: { ...input, flex: 1 }, value: w.name, onChange: e => updWin(i, "name", e.target.value), placeholder: "名称（如：5小时 / 余额）" }),
						h("select", { style: { ...input, width: "auto", fontSize: 10 }, value: w.type || "percent", onChange: e => updWin(i, "type", e.target.value) },
							h("option", { value: "percent" }, "百分比"),
							h("option", { value: "balance" }, "余额")),
						h("button", { type: "button", style: { ...btn, fontSize: 10, color: "var(--dsw-alias-state-danger-primary, #ef4444)" }, onClick: () => delWin(i) }, "✕")),
					(w.type || "percent") === "percent"
						? h("div", null,
							h("div", { style: { display: "flex", gap: 4, marginBottom: 3 } },
								...
								h("div", { style: { flex: 1 } }, h("input", { style: { ...input, fontSize: 10 }, value: w.resetsAtPath, onChange: e => updWin(i, "resetsAtPath", e.target.value), placeholder: "重置时间路径(可选)" }))),
						...
						: h("div", null,
							h("div", { style: { display: "flex", gap: 4, marginBottom: 3 } },
								h("div", { style: { flex: 1 } }, h("input", { style: { ...input, fontSize: 10 }, value: w.balancePath, onChange: e => updWin(i, "balancePath", e.target.value), placeholder: "剩余量路径" })),
								h("div", { style: { flex: 1 } }, h("input", { style: { ...input, fontSize: 10 }, value: w.totalPath, onChange: e => updWin(i, "totalPath", e.target.value), placeholder: "总量路径(可选)" }))),
							h("div", { style: { display: "flex", gap: 4 } },
								h("div", { style: { flex: 1 } }, h("input", { style: { ...input, fontSize: 10 }, value: w.usedPath, onChange: e => updWin(i, "usedPath", e.target.value), placeholder: "已用路径(可选)" })),
								h("div", { style: { flex: 1 } }, h("input", { style: { ...input, fontSize: 10 }, value: w.unit, onChange: e => updWin(i, "unit", e.target.value), placeholder: "单位(如 $ 或 点数)" }))))
				)),
